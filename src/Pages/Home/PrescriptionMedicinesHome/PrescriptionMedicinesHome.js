...
const PrescriptionMedicinesHome = () => {
    ...
    return (
        <div className='max-w-6xl mx-auto my-5 relative'>
            <h3 className='text-4xl font-bold text-center lg:text-left mb-3'>Prescription Medicines</h3>
            <div className='w-full grid grid-cols-2 lg:grid-cols-5 gap-6 py-8 px-12 sm:px-0 cursor-pointer'>
                {medicine.map(({ id, img, title, type, price }) => (
                    <div
                        key={id}
                        ...
                        <div className='pl-6'>
                            {
                                (title.length <= 15) ?
                                    ...
                                    :
                                    <p className="text-lg mt-4">{title.slice(0, 15)}...</p>
                            }
            ...
            <div className='text-center md:absolute top-0 right-0'>
                <Link to='/prescription-medicines'><PrimaryButton classes={`btn-sm normal-case hover:scale-105 duration-500`}><span className='text-xl'>View all</span></PrimaryButton></Link>
            </div>
        </div>
    );
};
